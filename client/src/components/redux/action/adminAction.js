...
export const registerAdmin=(user)=>async(dispatch)=>{
    ...
    try {
        ...
        const response=await axios.post("http://localhost:5000/admin/register",{name:user.name,email:user.email,password:user.password})
...
//         console.log(orderId);
//         const response=await axios.get(`http://localhost:5000/getOrder/${orderId}`)
//         console.log(response);
//         if(response.data.success){
//             dispatch({type:GET_SINGLE_ORDER_SUCCESS,payload:response});
//         }
//     } catch (error) {
//         console.log(error);
...
export const verifyAdmin=()=>async(dispatch)=>{
    try {
        ...
        const response=await axios.get("http://localhost:5000/adminAuth");
        // console.log(response);
        if(response.data.success){
...
export const updatedProduct=(product,id)=>async(dispatch)=>{
    try {
        ...
        const response=await axios.put(`http://localhost:5000/updateProduct/${id}`,product)
        if(response.data.success){
            const response=await axios.get("http://localhost:5000/getAllProducts");
            if(response.data.success){
                dispatch({type:FETCH_PRODUCTS_SUCCESS,payload:response.data.Products});
...
export const changeOrderStatus=(orderStatus,id)=>async(dispatch)=>{
    try {
        ...
        const response=await axios.post(`http://localhost:5000/order-status/${id._id}`,{orderStatus});
        // console.log(response);
        if(response.data.success){
...
export const deleteOrder=(id)=>async(dispatch)=>{
    try {
        ...
        const response=await axios.delete(`http://localhost:5000/delete-order/${id}`)
        if(response.data.success){
...
export const addCoupon=(coupon)=>async(dispatch)=>{
    try {
        ...
        const response=await axios.post(`http://localhost:5000/make-coupon`,coupon);
        if(response.data.success){
            const response=await axios.get(`http://localhost:5000/allCoupons`);
            if(response.data.success){
...
export const deleteCoupon=(id)=>async(dispatch)=>{
    try {
        ...
        const response=await axios.delete(`http://localhost:5000/delete-coupon/${id}`);
        if(response.data.success){
            const response=await axios.get(`http://localhost:5000/allCoupons`);
            if(response.data.success){
                dispatch({type:GET_ALL_COUPONS_SUCCESS,payload:response.data.coupons});
