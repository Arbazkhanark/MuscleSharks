...
export const deleteFromCart=(id)=>async(dispatch)=>{
    ...
    try {
        ...
        const response=await axios.delete(`http://localhost:5000/cart/deleteProduct/${id}`);
...
export const addToCart=(cartData)=>async(dispatch)=>{
    ...
    try {
        const response=await axios.post(`http://localhost:5000/addCart`,{productId:cartData.productId,quantity:1,selectedVariantId:cartData.selectedVariantId});
        if(response.success){
...
export const updateCart=(variantId,quantity)=>async(dispatch)=>{
    ...
    try {
        ...
        const response=await axios.put(`http://localhost:5000/cart-update?variantId=${variantId}`,{quantity})
        if(response.data.success){
            const response=await axios.get("http://localhost:5000/getCart");
            // console.log("res",response.data);
            if(response.data.success){
