...
export const addReview=(id,review,rating)=>async(dispatch)=>{
    try {
        ...
        const response=await axios.post(`http://localhost:5000/review/${id}`,{comment:review,rating});
        // console.log(response);
        if(response.data.success){
            const response=await axios.get(`http://localhost:5000/getReview/${id}`)
            // console.log(response);
            if(response.data.success){
...
export const fetchSortedProducts=(query)=>async(dispatch)=>{
    try {
        ...
        console.log(query);
        const response=await axios.get(`http://localhost:5000/products/sort/${query}`);
        if(response.data.success){
