...
export const userRegistrationAction=(user,navigate)=>async(dispatch)=>{
    try {
        ...
        if(response.data.success){
            ...
            dispatch({type:USER_REGISTER_FAILED,payload:response.data.error})
...
export const updateUserProfile=(user)=>async(dispatch)=>{
    try {
        ...
        const response=await axios.put("http://localhost:5000/updateProfile",{user});
        // console.log(response);
        if(response.data.success){
...
export const forgetPassword=(email)=>async(dispatch)=>{
    ...
    try {
        ...
        const response=await axios.put(`http://localhost:5000/forget-password`,{email});
...
export const resetPassword=(user,token)=>async(dispatch)=>{
    try {
        ...
        const response=await axios.post(`http://localhost:5000/reset-password?token=${token}`,{newPassword:user.newPassword,confirmPassword:user.confirmPassword})
        // console.log(response);
...
export const userAuth=()=>async(dispatch)=>{
    try {
        ...
        const response=await axios.get("http://localhost:5000/verifyingUser");
...
export const verifyOtp=(data,navigate)=>async(dispatch)=>{
    ...
    try {
        ...
        const response=await axios.post("http://localhost:5000/verify-account",{otp,email:data.email});
        // console.log(response);
        if(response.data.success){
...
export const getSingleOrder=()=>async(dispatch)=>{
    try {
        ...
        const response=await axios.get("http://localhost:5000/getAllOrdersUser")
        // console.log(response);
        if(response.data.success){
